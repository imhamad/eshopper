...
class rehub_tabs_widget extends WP_Widget {
    ...
    function __construct() {
		$widget_ops = array( 'classname' => 'tabs', 'description' => esc_html__('A widget that displays 2 tabs (popular, categories, tags, latest comments). Use only in sidebar! ', 'rehub-framework') );
	...
	function form( $instance ) {
		...
		<label for="<?php echo ''.$this->get_field_id('basedby'); ?>"><?php esc_html_e('Popular posts for tab 1 based on:', 'rehub-framework');?></label> 
		<select id="<?php echo ''.$this->get_field_id('basedby'); ?>" name="<?php echo ''.$this->get_field_name('basedby'); ?>" style="width:100%;">
			<option value='comments' <?php if ( 'comments' == $instance['basedby'] ) : echo 'selected="selected"'; endif; ?>><?php esc_html_e('Comments', 'rehub-framework');?></option>
			<option value='views' <?php if ( 'views' == $instance['basedby'] ) : echo 'selected="selected"'; endif; ?>><?php esc_html_e('Post views', 'rehub-framework');?></option>
		</select>
		<span><em><?php esc_html_e('Note, post views may not work if you use cache plugins!', 'rehub-framework');?></em></span>		
		</p>				
		<p class="tabbaserehubsecond">
		<label for="<?php echo ''.$this->get_field_id('basedbysec'); ?>"><?php esc_html_e('Popular posts for tab 2 based on:', 'rehub-framework');?></label> 
		<select id="<?php echo ''.$this->get_field_id('basedbysec'); ?>" name="<?php echo ''.$this->get_field_name('basedbysec'); ?>" style="width:100%;">
			<option value='comments' <?php if ( 'comments' == $instance['basedbysec'] ) : echo 'selected="selected"'; endif; ?>><?php esc_html_e('Comments', 'rehub-framework');?></option>
			<option value='views' <?php if ( 'views' == $instance['basedbysec'] ) : echo 'selected="selected"'; endif; ?>><?php esc_html_e('Post views', 'rehub-framework');?></option>
		</select>
		<span><em><?php esc_html_e('Note, post views may not work if you use cache plugins!', 'rehub-framework');?></em></span>		
		</p>						
		<p>
