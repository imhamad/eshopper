...
class VP_Metabox extends WPAlchemy_MetaBox
{
	...
	function __construct($arr)
	{
		...
		if( $this->is_dev_mode )
		{
			$this->title = esc_html__('[Development Mode] ', 'rehub-framework') . $this->title;
		}
	...
	function _render_repeating_group($group)
	{
		...
		foreach ($group['groups'] as $g)
		{
			...
			$html .= '<div class="vp-wpa-group-heading"><a href="#" class="vp-wpa-group-title">' . $icon . $group['title'] . '</a><a href="#" class="dodelete vp-wpa-group-remove" title="'. esc_html__('Remove', 'rehub-framework') .'"><i class="far fa-times"></i> '. esc_html__('Remove', 'rehub-framework') .'</a></div>';
			$html .= '<div class="vp-controls' . ((!$is_first) ? ' vp-hide' : '') . '">';
		...
		$html .= '<a href="#" class="button button-primary docopy-' . $name . '">'. esc_html__('Add More', 'rehub-framework') . ' ' . $group['title'] . '</a>';
		$html .= '</div>';
